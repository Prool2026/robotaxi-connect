...
export default async function Provider({params}:{params:Promise<{locale:string}>}) {
 ...
 <AccountInput name="company_name" label={de?'Firmenname':'Company name'} value={p.company_name||''}/><AccountInput name="email" type="email" label={de?'Geschäftliche Kontakt-E-Mail':'Business contact email'} value={p.email||user.email||''}/><AccountInput name="phone" type="tel" label={de?'Telefon':'Phone'} value={p.phone||''} max={50}/><AccountInput name="address" label={de?'Geschäftsanschrift':'Business address'} value={p.address||''} max={500}/><AccountInput name="website" type="url" label="Website" value={p.website||''} required={false}/><label className="field">{de?'Technologie / Lösung':'Technology / solution'}<textarea name="solution" defaultValue={p.solution||''} rows={4} maxLength={5000} required/></label><label className="field">{de?'Zielmärkte und Entwicklungsstand':'Target markets and development stage'}<textarea name="markets" defaultValue={p.markets||''} rows={3} maxLength={2000}/></label><h3>{de?'Ihre Lösung im praktischen Einsatz':'Your solution in practice'}</h3><p>{de?'Bitte beantworten Sie die folgenden Fragen möglichst konkret. Falls etwas noch offen ist, tragen Sie „Noch in Planung“ oder „Nicht zutreffend“ ein. Wir nutzen Ihre Angaben für die persönliche Auswahl passender Partner.':'Please be as specific as possible. If a point is still open, enter “Still being planned” or “Not applicable”. We use your answers to personally identify suitable partners.'}</p>{providerQuestions.map(q=><label className="field" key={q.key}>{q[locale]}<textarea name={q.key} defaultValue={p.qualification?.[q.key]||''} rows={3} maxLength={2500} required/></label>)}</AccountForm></section>
